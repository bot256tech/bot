...
router.post('/issue', protect(['PARTNER', 'ADMIN', 'lab', 'quality_officer', 'admin']), async (req, res) => {
  try {
    const passport = await QualityService.issueQualityPassport(req.body);
    res.status(201).json({
      success: true,
...
router.get('/verify/:batch_number', async (req, res) => {
  try {
...
router.put('/update/:id', protect(['PARTNER', 'ADMIN', 'lab', 'quality_officer', 'admin']), async (req, res) => {
  try {
    const { moisture_level, aflatoxin_result, quality_grade } = req.body;

    if (!moisture_level || !aflatoxin_result) {
      return res.status(400).json({
        success: false,
        message: 'Both moisture_level and aflatoxin_result are required.'
      });
    }
...
router.get('/farmer/:farmer_id', protect(), async (req, res) => {
  try {
